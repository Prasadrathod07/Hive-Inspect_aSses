...
| 14 | Public deployment must work on Vercel | Deployed at https://hive-inspect-a-sses-sepia.vercel.app/ from the `main` branch. Every route declares `export const runtime = "nodejs"`; `xlsx` confirmed pure JavaScript (no native bindings); zero filesystem or localStorage dependency in the deployed app; no hardcoded localhost/dev URLs. A genuine, previously-undetected bug was found and fixed during prep: Vercel enforces a hard, non-configurable 4.5MB request body limit on every plan, but this app advertised and accepted uploads "up to 20MB" — fixed by lowering `MAX_FILE_SIZE_BYTES` to 4MB everywhere, derived from one constant (`docs/decision-log.md` D15). A second real deploy-time failure was found and fixed after the first deploy attempt: `vitest@5.0.1`'s peer dependency required `@types/node@^22`, but `package.json` had it pinned to `^20` (also inconsistent with the project's own `engines.node: "22.x"`) — `npm install` failed on Vercel with `ERESOLVE` until this was corrected | `npm run build` succeeds; `GET /api/health` on the live URL confirmed returning `{"status":"ok","checks":{"supabaseConfigured":true,"supabaseReachable":true}}`; `.next/static` scanned and confirmed free of the service-role key and all other secrets; `.env.example` audited against every actual `process.env` reference in the codebase | DONE — live and reachable; still needs row 15's seed step for a reviewer to have something to open |
...
- Row 13 is now **BLOCKED** (was TODO): the schema, atomic persistence
  ...
  `ENGINEERING.md` §5's "works end-to-end against the real Supabase backend, not
  a mock."
...
- The Templates dashboard (`/`) and import workflow (`/import`) are now real,
  ...
  still TODO, and ENGINEERING.md prohibits half-finished implementations, so the
...
- The import report and Issue Review pages are now fully wired to the real
  ...
  the half-finished-feature pattern ENGINEERING.md prohibits.
...
- Rows 17/18 moved to **DONE** with `README.md` and `NOTES.md`, both written
  ...
  caught along the way). `ENGINEERING.md` is kept, not superseded, as the
